...
A.Moon = {
	 ...
	 */
	topocentricPosition: function (jdo, eclCoord, refraction, azType) {
		// azType = "n" or "N" for azimuth calculated from Nort (default: from South)
		var st0 = A.Sidereal.apparentInRa(jdo);
		var aet = A.Moon.apparentTopocentric(jdo, eclCoord, st0);
		
		var hz = A.Coord.eqToHz(aet.eq, eclCoord, st0, azType);
	 ...
	 * Lat/Lon are in ecliptic coordinates, use eclToEq() (--> Ra/Dec)  and eqToHz() (Alt/Az) to convert 
	 * Check result against:  https://www.satellite-calculations.com/Satellite/suncalc.htm)
	 * 
	...
	geocentricPosition: function (jdo) {
		...
		return {
			...
			lat: sumb * 1e-6 * p,                          
			delta: 385000.56 + sumr*1e-3                   // Distance from Earth in km
		};
	},
